#!/bin/bash
# Deploy fixed chain spec to all VMs
...
echo "Copying fixed chain spec from build VM..."
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no "root@$BUILD_VM:/tmp/flarechain-mainnet-v108-raw-fixed.json" /tmp/chainspec-fixed.json
...
for VM in "${VMS[@]}"; do
    ...
    # Deploy chain spec
    cat /tmp/chainspec-fixed.json | ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "root@$VM" \
...
echo ""
echo "All VMs updated! Waiting 20 seconds for services to start..."
sleep 20
